...
def import_dashboard(
    ...
    for slc in slices:
        ...
        if database_id:
            database_name = db.session.query(Database).filter(Database.id == database_id).first().name
...
def medbi_import_dashboards(
    ...
    db.session.commit()
    for dashboard in data["dashboards"]:
        medbi_import_dashboard(
            dashboard, dataset_id_mapping, import_time=import_time,
            database_id=database_id,
            clickhouse_database_id=clickhouse_database_id
        )
    db.session.commit()
...
class MedbiImportDashboardsCommand(BaseCommand):
    ...
    def run(self) -> None:
        ...
        for file_name, content in self.contents.items():
            logger.info("Importing dashboard from file %s", file_name)
            medbi_import_dashboards(content,
                                    self.database_id,
